...
def assembly_multiruns(config, session, jenkins_build_url=None):
    ...
    for run in unused_complete_runs:
        ...
        logger.debug("Getting list of ALCAPROMPT datasets for run {} from DBS".format(run.number))
        run_datasets = dbsApi.listAlCaPromptDatasets(run_num=run.number)
        ...
        for dataset in datasets:
            ...
            # since we iterate through runs, the same url can be repeated for the same multi-run
            # we want to avoid it
